...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "9. list_trait_categories — discover the 35 curated natural-"
        ...
        "tillering, aluminum tolerance, plus cannabis-, maize-, and "
        "orphan-crop-specific categories).\n"
        ...
        "COMMUNITY SCIENCE LAYER (the write path): the tools above are "
        "read-only. The tools below let grower-scientists CONTRIBUTE "
        "observations and reason about collective statistical power — the "
        "commons grows here. Ledger data is licensed ODbL-1.0 (open-data "
        "copyleft, see DATA_LICENSE.md), distinct from the Apache-2.0 code "
        "license. Attribution is via Ed25519 signatures — a scientific-"
        "credit mechanism, NOT a token or cryptocurrency.\n"
        "- submit_phenotype_observation — record a measured field phenotype "
        "(e.g. 'this landrace survived 14-day flooding'). Validates the trait "
        "category + species, writes a schema-v1.0 YAML to the ledger, and "
        "returns a canonical form to sign + PR instructions. It prepares the "
        "artifact; the human opens the PR (no GitHub credentials at runtime). "
        "If the user gives an informal seed name, run resolve_accession FIRST.\n"
        "- query_community_phenotypes — aggregate the ledger for a trait: "
        "count, measurement distribution, accessions, signed-observation count.\n"
        "- estimate_gwas_power — 'can my village's 12 varieties detect this "
        "locus?' Bonferroni-corrected additive power calc; pulls the live "
        "ledger count when n_observations is omitted. Order-of-magnitude "
        "guidance — surface the caveat that real GWAS needs kinship/structure "
        "correction, which raises required N.\n"
        "- verify_observation_integrity — verify a detached Ed25519 signature "
        "over the canonical observation. Unsigned observations are still valid "
        "data — they just lack an attribution proof.\n"
        "- pin_observation_to_ipfs — content-address an observation (+ optional "
        "VCF) via a local kubo node; structured fallback if none is reachable.\n"
        "- resolve_accession — bridge a folk seed name ('my grandmother's Hopi "
        "blue corn') to a USDA GRIN-Global accession + an Ensembl species "
        "string. Run before lookup_gene / submit_phenotype_observation when the "
        "name is informal.\n"
        "- query_organellar_variants — Mt (mitochondrion) and Pt (plastid) are "
        "VALID Ensembl chromosomes, not errors. Curated organellar atlas: cms "
        "(cytoplasmic male sterility — how hybrid seed is made), "
        "plastid_herbicide_resistance (psbA / atrazine), plastid_photosynthesis "
        "(rbcL). Organellar genomes are maternally inherited.\n"
        "- export_offline_snapshot — package a trait's atlas entry + species "
        "coverage + optional orthologs into a portable JSON for the offline "
        "TinyLLamaFarmer (online->offline; the reverse is "
        "submit_phenotype_observation).\n"
        "- list_orphan_crop_requests — the orphan-crop contribution bounty list "
        "(teff, fonio, cowpea, pigeon pea, finger millet, amaranth, etc.).\n"
        "Also: get_variant and search_variants_in_region accept "
        "population_context=True for per-population allele frequencies + "
        "Indica/Japonica Fst on richly-covered species.\n\n"
